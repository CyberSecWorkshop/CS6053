...
def err_ratio(learner, dataset, examples=None):
    ...
    Handles both object-based learners (with .predict()) and function-based
    learners (e.g. NaiveBayesLearner which returns a callable directly).
    """
    ...
    for example in examples:
        ...
        # Support both learner.predict(x) and learner(x)
        if hasattr(learner, 'predict'):
            output = learner.predict(dataset.sanitize(example))
        else:
            output = learner(dataset.sanitize(example))
        if output == desired:
            right += 1
...
class DecisionLeaf:
    ...
    def display(self, indent=0):  # FIX: added indent=0 parameter
